...
public:
  ...
  //void query(const BowVector &vec, QueryResults &ret, 
   // int max_results = 1, std::vector<int> candidate_set) const;
  ...
  QueryResults &ret, int max_results, int max_id) const
{
  ...
  for(vit = vec.begin(); vit != vec.end(); ++vit)
  {
    ...
    for(rit = row.begin(); rit != row.end(); ++rit)
    {
      ...
      if((int)entry_id < max_id and (int)entry_id > min_id_) // || (int)entry_id == m_nentries - 1 //这里不知道为什么会有这个条件
      {
  ...
  QueryResults &ret, int max_results, int max_id) const
{
  ...
  for(vit = vec.begin(); vit != vec.end(); ++vit)
  {
    ...
    for(rit = row.begin(); rit != row.end(); ++rit)
    {
      ...
      if(candidate_set_.size()>1)
      {
          auto it = std::find(candidate_set_.begin(), candidate_set_.end(), (int)entry_id);
          //std::cout<<"entry_id "<<(int)entry_id<<" "<<candidate_set_[0]<<" "<<candidate_set_[1]<<endl;
          if(it != candidate_set_.end())
             is_candidate = true;
      }else{
      ...
      if(is_candidate || max_id == -1 || (int)entry_id == m_nentries - 1)
      {
        double value = fabs(qvalue - dvalue) - fabs(qvalue) - fabs(dvalue);
