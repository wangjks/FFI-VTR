...
void ransac_run1(std::vector<cv::DMatch> matches, std::vector<cv::KeyPoint> queryKeypoint, std::vector<cv::KeyPoint> objectKeypoint, std::vector<cv::DMatch> &matches_ransac)
{
    ...
    for(int i=0;i<matches.size();i++){
        ...
        if(abs(srcPoints[i].y - dstPoints[i].y) < 200)
            inliersMask[i] = 1;
        else
            inliersMask[i] = 0;
    ...
    // cv::Mat essential_mat = cv::findEssentialMat(srcPoints, dstPoints, cameraMatrix, cv::RANSAC, 0.999, 1.0, inliersMask);
        ...
        if(load_previous_pose_graph)
        {
            ...
            if(max_inlier_size>15 or true)
            {
                    ...
                    // flow = candi_flow_set_[inlier_id];
                        global_loop_index = candidate_index_set_[0];
                        ...
                        {
                                // break;
                                KeyFrame* next_kf = getKeyFrame(cur_kf->loop_index+1*k); //得到闭环帧
                                if(next_kf  != nullptr)
                                {
                                    if(k>0)
                                    {
                                        flow_set.push_back(next_kf->flow_);
                                        continue;
                                    }
                                    std::vector<cv::DMatch> matches2;
                                    std::vector<cv::Point2f> points1;
                        ...
                        if(cur_kf->index%1==0 and true)
                        {
                                ...
                                ofstream outfile;
                                string traj_file = "/home/wjk/catkin_flowfusion/output/flowrecord.txt";
                            
                                //cout<<"traj_file: "<<traj_file.c_str()<<endl;
                                outfile.open(traj_file.c_str(),ios::app);
                                outfile<<cur_kf->index<<" "<<move_s_prob<<" "<<move_left_prob<<" "<<move_right_prob<<
                                " "<<flow_set[0]<<" "<<flow_set[1]<<" "<<flow_set[2]<<std::endl;
                                outfile.close();
                                // waitKey(0);
                        }
                    ...
                    if(abs(global_loop_index-map_size_)<6)
                    {
                        current_goal_stamped.point.x = 0.0;
                        current_goal_stamped.point.y = 0.0;
                        current_goal_stamped.header.stamp = ros::Time::now();
...
int PoseGraph::detectLoop(KeyFrame* keyframe, int frame_index)
{
    ...
    max_id = global_loop_index+15;//wjk new added 0226
    db.min_id_ = max(0,global_loop_index - 2);
